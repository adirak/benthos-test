...
public class BloblangSensor implements Sensor {
    ...
    public void execute(SensorContext context) {
        ...
        // Analyze pure Bloblang files
        analyzeBloblangFiles(context, fileSystem);
        
        // Analyze YAML files that contain Bloblang content
        analyzeYamlFiles(context, fileSystem);
    }
    ...
    private void analyzeBloblangFiles(SensorContext context, FileSystem fileSystem) {
        ...
        for (InputFile inputFile : bloblangFiles) {
            analyzeFile(context, inputFile, "Bloblang");
        }
    }

    private void analyzeYamlFiles(SensorContext context, FileSystem fileSystem) {
        Iterable<InputFile> yamlFiles = fileSystem.inputFiles(
            fileSystem.predicates().and(
                fileSystem.predicates().hasType(InputFile.Type.MAIN),
                fileSystem.predicates().matchesPathPatterns(new String[]{"**/*.yaml", "**/*.yml"})
            )
        );

        for (InputFile inputFile : yamlFiles) {
            try {
                String content = inputFile.contents();
                if (BloblangLanguage.isBloblangYaml(content)) {
                    analyzeFile(context, inputFile, "YAML-Bloblang");
                }
            } catch (IOException e) {
                LOG.warn("Cannot read YAML file: " + inputFile.filename(), e);
            }
        }
    }

    private void analyzeFile(SensorContext context, InputFile inputFile, String fileType) {
        try {
            String content = inputFile.contents();
            ...
            LOG.info("Analyzed {} file: {} - Lines: {}, NCLOC: {}, Comments: {}", 
                fileType, inputFile.filename(), metrics.lines, metrics.linesOfCode, metrics.commentLines);
